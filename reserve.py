...
def split_image_into_grid(image, grid_size=(3, 3)):
    rows, cols = grid_size
    h, w, _ = image.shape
...
def reassemble_image_from_grid(split_images, original_image_size):
    if len(split_images) != 9:
        raise ValueError("There should be exactly 9 images to reassemble")
    ...
    for i in range(rows):
        for j in range(cols):
            start_row, start_col = i * grid_h, j * grid_w
            reassembled_image[start_row:start_row + grid_h, start_col:start_col + grid_w] = split_images[i * cols + j]
...
def run(args, data):
    ...
    for path in data:
        ...
        split_images = split_image_into_grid(original_image)
        processed_images = []
        ...
        for i in range(rows):
            for j in range(cols):
                index = i * cols + j
                img = split_images[index]
                start_row, start_col = i * grid_h, j * grid_w  # Calculate the grid's starting position
                ...
                # Pass the grid offset to process_model_output
                processed_img, num_holes, num_cracks = process_model_output(output, img, args, start_row, start_col)
        ...
        # Reassemble the full image from processed grid sections
        reassembled_image = reassemble_image_from_grid(processed_images, original_image.shape[:2])
...
def process_model_output(output, img, args, start_row, start_col):
