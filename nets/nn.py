...
class DarkFPN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # Upsample and concatenate p5 and p4
        upsampled_p5 = self.up(p5)
        p4 = self._pad_to_match(p4, upsampled_p5)
        p4 = self.h1(torch.cat([upsampled_p5, p4], 1))

        # Upsample and concatenate p4 and p3
        upsampled_p4 = self.up(p4)
        p3 = self._pad_to_match(p3, upsampled_p4)
        p3 = self.h2(torch.cat([upsampled_p4, p3], 1))

        # Upsample p3 for the next concatenation
        upsampled_p3 = self.h3(p3)
        p4 = self._pad_to_match(p4, upsampled_p3)
        p4 = self.h4(torch.cat([upsampled_p3, p4], 1))

        # Upsample p4 for the next concatenation
        upsampled_p4 = self.h5(p4)
        p5 = self._pad_to_match(p5, upsampled_p4)
        p5 = self.h6(torch.cat([upsampled_p4, p5], 1))

        return p3, p4, p5

    def _pad_to_match(self, lower_feature, higher_feature):
        """
        Pads the lower_feature tensor on all sides to match the dimension of the higher_feature tensor.
        """
        # Calculate padding for the lower feature to match the higher feature
        height_diff = higher_feature.size(2) - lower_feature.size(2)
        width_diff = higher_feature.size(3) - lower_feature.size(3)
        
        # Apply symmetric padding if necessary
        padding = [width_diff // 2, width_diff - width_diff // 2,
                   height_diff // 2, height_diff - height_diff // 2]
        return F.pad(lower_feature, padding, "constant", 0)
