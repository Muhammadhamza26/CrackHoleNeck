...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if label.size:
                label[:, 1:] = wh2xy(label[:, 1:], ratio[0]
                                     * w, ratio[1] * h, pad[0], pad[1])
            if self.augment:
    ...
    def load_label(filenames):
        ...
        for filename in filenames:
            try:
                ...
                assert image.format.lower(
                ) in FORMATS, f'invalid image format {image.format}'
...
def resize(image, input_size, augment):
    ...
    image = cv2.copyMakeBorder(
        image, top, bottom, left, right, cv2.BORDER_CONSTANT)  # add border
...
def candidates(box1, box2):
    ...
    aspect_ratio = numpy.maximum(
        w2 / (h2 + 1e-16), h2 / (w2 + 1e-16))  # aspect ratio
...
def random_perspective(image, label, params, border=(0, 0)):
    ...
    translate[0, 2] = random.uniform(
        0.5 - params['translate'], 0.5 + params['translate']) * w
    translate[1, 2] = random.uniform(
...
class Albumentations:
    def __init__(self):
        ...
        try:
            ...
            transforms = [albumentations.Blur(p=0.05),
                          albumentations.CLAHE(p=0.05),
